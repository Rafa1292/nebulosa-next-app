import { getPaginatedInputs } from '@/actions'
      ...
      >
        {inputs.map((input) => (
          <tr key={input.id} className='bg-white border-b transition duration-300 ease-in-out hover:bg-gray-100'>
            ...
            <td className='text-sm font-bold text-gray-900 px-6 py-4 text-center whitespace-nowrap'>
              {measures.find((x) => x.slug === input.measureSlug)?.name}
            </td>
            <td className='text-sm font-bold text-gray-900 px-6 py-4 justify-center flex gap-6'>
              ...
              <Link href={`/admin/inputs/${input.id}/view`} className='font-bold text-2xl cursor-pointer'>

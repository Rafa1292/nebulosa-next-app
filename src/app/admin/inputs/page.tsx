  ...
  return (
    <>
      <TableList
        ...
        heads={['Nombre', 'Precio', 'Presentacion', '']}
      >
        {inputs.map((input) => (
          <tr key={input.id} className='bg-white border-b transition duration-300 ease-in-out hover:bg-gray-100'>
            <td className='text-sm font-bold text-gray-900 px-6 py-4 whitespace-nowrap'>{input.name}</td>
            <td className='text-sm font-bold text-gray-900 px-6 py-4 whitespace-nowrap'>{input.expectedPrice}</td>
            <td className='text-sm font-bold text-gray-900 px-6 py-4 whitespace-nowrap'>{input.presentation}</td>

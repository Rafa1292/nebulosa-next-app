...
import { currencyFormat, getCurrentBillItemTotal } from '@/utils'
...
export const BillItemUI = ({ billItem, handleEditBillItem, currentKey, bill }: Props) => {
  ...
  return (
    <div
      key={currentKey}
      className='flex flex-wrap mb-2 text-black transition-all cursor-pointer select-none items-center px-0  shadow-md rounded-md '
    >
      ...
      <div className={`${titleFont.className}  antialiased text-center w-1/6 text-xs font-bold`}>
        {currencyFormat(
          bill !== undefined
            ? getCurrentBillItemTotal(bill, billItem.saleItemId)
            : getBillItemTotal(billItem.saleItemId) - billItem.discount
        )}
      </div>
        ...
        {handleEditBillItem && (
          <>
            ...
            <RiEdit2Fill
              ...
              className={clsx('text-black', 'hover:!text-gray-500')}
              size={20}
            />
          </>
        )}
      ...
      >
        {billItem
          .itemArticles!.sort((a, b) => a.itemNumber - b.itemNumber)
          .map((itemArticle, index) => (
            <div
              key={index}
            ...
            >
              {itemArticle.linkedArticles !== undefined && (
                <>
                  ...
                  <div
                    className={`${subTitleFont.className}  antialiased text-black text-center w-1/6 text-xs font-bold`}
                  >
